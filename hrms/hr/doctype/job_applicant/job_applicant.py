...
class JobApplicant(Document):
    ...
    def validate(self):
        ...
        if frappe.db.exists(
            ...
            frappe.throw(
                _(
                    "You have already applied for this job and your application is still pending."
                ),
                title=_("Not Allowed"),
            )
    ...
    def before_insert(self):
        if self.job_title:
            job_opening_status = frappe.db.get_value(
                "Job Opening", self.job_title, "status"
            )
...
def create_interview(doc, interview_round):
    ...
    round_designation = frappe.db.get_value(
        "Interview Round", interview_round, "designation"
    )
...
def get_interview_details(job_applicant):
    ...
    for detail in interview_details:
        detail.average_rating = (
            detail.average_rating * number_of_stars if detail.average_rating else 0
        )
...
def get_applicant_to_hire_percentage():
    ...
    return {
        "value": (
            flt(total_hired) / flt(total_applicants) * 100 if total_applicants else 0
        ),
        "fieldtype": "Percent",
    }
